...
def upload_asset(file_bytes, api_key):
    ...
    res = requests.post('https://api.krea.ai/assets', headers=headers, files=files, timeout=30)
...
def show_editor():
    ...
    with col1:
        ...
        muro_file = st.file_uploader("Sube la foto del muro/espectacular", type=["jpg", "jpeg", "png"], key="muro_upload")
        if muro_file:
    ...
    with col2:
        ...
        logo_file = st.file_uploader("Sube el logo o creatividad", type=["jpg", "jpeg", "png"], key="logo_upload")
        if logo_file:
    ...
    if st.button("Generar Imagen / Refinar", icon=":material/auto_fix_high:", type="primary", use_container_width=True):
        if not st.session_state.krea_api_key:
            st.error("Error de conexión con el servicio de generación de imágenes. Contacta al administrador.", icon=":material/error:")
        elif not muro_file or not logo_file:
            st.error("Debes subir ambas imágenes (el soporte y el logo).", icon=":material/warning:")
        elif not user_prompt:
            st.error("Debes ingresar un prompt o instrucción.", icon=":material/warning:")
        else:
            with st.spinner(f"Procesando las imágenes y enviando a KREA con {selected_model}..."):
                try:
                    ...
                    if st.session_state.gemini_api_key:
                        st.info("Refinando el prompt con Gemini...", icon=":material/psychology:")
...
El prompt debe describir cómo integrar el logo de forma realista sobre el espacio asignado en el muro, ajustando la perspectiva, las sombras, y la iluminación para que parezca una integración real, no un pegado. No incluyas texto de relleno, solo devuelve el prompt mejorado en un solo párrafo."""
                        ...
                        st.success("Prompt Refinado Exitosamente", icon=":material/check_circle:")
                        st.info(f"**Prompt Mejorado:** {refined_prompt}")
                    else:
                        st.info("El prompt se enviará a KREA sin refinamiento previo.", icon=":material/info:")
...
Instrucción a seguir: {refined_prompt}"""
                    ...
                    st.success("Imágenes subidas. Iniciando generación...", icon=":material/check_circle:")
                    ...
                    for i in range(60):
                        ...
                        if status == 'completed':
                            ...
                            if result_urls:
                                st.success("¡Generación completada!", icon=":material/check_circle:")
                                st.image(result_urls[0], caption=f"Imagen generada por {selected_model}", use_container_width=True)

                                try:
                                    img_response = requests.get(result_urls[0], timeout=15)
                                    ...
                                    st.warning("No se pudo crear el botón de descarga. Puedes guardar la imagen con click derecho.", icon=":material/warning:")
                            else:
                                st.warning("El modelo completó el trabajo pero no devolvió ninguna URL.", icon=":material/warning:")
                            completed = True
                            break
                        elif status in ['failed', 'cancelled']:
                            st.error(f"La generación falló o fue cancelada por KREA. Estado: {status}", icon=":material/error:")
                            completed = True
                            break
                    ...
                    if not completed:
                        st.warning("Tiempo de espera agotado consultando a KREA.", icon=":material/warning:")

                except Exception as e:
                    st.error(f"**Ocurrió un error:** {e}", icon=":material/error:")
    ...
    with col_adj1:
        adjustment = st.text_input("Ajuste adicional:", placeholder="Ej. Mantén la posición pero ajusta mejor las sombras", label_visibility="collapsed", key="adjust_input")
    with col_adj2:
        if st.button("Ajustar Resultado", icon=":material/tune:", use_container_width=True, key="adjust_btn"):
            st.info("Esta función tomará la imagen anterior y el nuevo prompt para perfeccionar el resultado.", icon=":material/build:")
